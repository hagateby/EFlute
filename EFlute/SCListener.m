...
- (BOOL)isListening {
	...
	OSStatus result = AudioQueueGetProperty(queue, kAudioQueueProperty_IsRunning, &isListeningNumber, &ioDataSize);
	return (result != noErr) ? NO : isListeningNumber;
}
...
- (void)setupQueue {
	...
	UInt32 audioRouteOverride = kAudioSessionOverrideAudioRoute_Speaker;//3
